...
static unsigned int count, _buff_size;
...
int csp_buffer_init(int buf_count, int buf_size) {
	...
	count = buf_count;
	_buff_size = buf_size;
	unsigned int skbfsize = (sizeof(csp_skbf_t) + _buff_size);
...
void *csp_buffer_get_isr(size_t buf_size) {
	...
	if (buf_size + CSP_BUFFER_PACKET_OVERHEAD > _buff_size)
		return NULL;
...
void *csp_buffer_clone(void *buffer) {
	...
	if (clone)
		memcpy(clone, packet, _buff_size);
